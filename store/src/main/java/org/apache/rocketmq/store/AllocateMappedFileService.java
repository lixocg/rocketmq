...
public class AllocateMappedFileService extends ServiceThread {
    ...
    //用于保存当前所有待处理的分配请求，其中key为filePath，value为分配请求，如果分配请求被成功处理，即获取到映射文件，则请求
    //会从requestTable中移除
    ...
    public MappedFile putRequestAndReturnMappedFile(String nextFilePath, String nextNextFilePath, int fileSize) {
        ...
        if (this.messageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
            if (this.messageStore.getMessageStoreConfig().isFastFailIfNoBufferInStorePool()
                    && BrokerRole.SLAVE != this.messageStore.getMessageStoreConfig().getBrokerRole()) { //如果broker为从节点，那么即使池子中没有buffer，也不快速失败
        ...
        /**
         * 如果requestTable中已经存在该路径文件的分配请求，说明请求已经在排队中，就不需要再次检查transientStorePool中的buffer是否够用，
         * 以及向requestQueue队列中添加分配请求
         */
        if (nextPutOK) {
            //如果transientStorePool中buffer不够了，快速失败
    ...
    /**
     * 创建内存映射文件
     * 该方法只有被外部线程中断，才会返回false
     */
    private boolean mmapOperation() {
        boolean isSuccess = false;
        AllocateRequest req = null;
        try {
            //检索并删除此队列的首节点，必要时等待，只带有可用元素
    ...
    static class AllocateRequest implements Comparable<AllocateRequest> {
        ...
        //用于实现分配映射文件的等待通知线程模型，初始值为1，0代表完成映射文件的创建
